...
describe("slidein internal enhancer", () => {
  beforeEach(() => {
    vi.resetModules();
  ...
  it("enhances a root using dataset threshold and root margin", async () => {
    const { enhanceSlideIn } = await import("../src/internal/slidein");
    const root = document.createElement("div");
    root.dataset.once = "false";
    root.dataset.threshold = "0.4";
    root.dataset.rootMargin = "12px";
    document.body.append(root);

    const events: string[] = [];
    root.addEventListener("astro:slidein:init", () => events.push("init"));
    root.addEventListener("astro:slidein:visible", () => events.push("visible"));
    root.addEventListener("astro:slidein:hidden", () => events.push("hidden"));
  ...
  it("boots existing and newly added enhanced roots once", async () => {
    const { bootSlideIn } = await import("../src/internal/slidein");
    const first = document.createElement("div");
    first.setAttribute("data-astro-slide-in", "");
    ...
    dynamic.setAttribute("data-astro-slide-in", "");
